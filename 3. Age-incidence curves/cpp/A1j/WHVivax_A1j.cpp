...
VivaxBrood::UpdResult VivaxBrood::update(LocalRng& rng){
    ...
    while( releaseDates.size() > 0 && releaseDates.back() == sim::ts0() ){
        ...
        //  comment out for total relapses model version, used only in nonw version
        ...
        double ageInYears)
{
    ...
    while( inf != infections.end() ){
        ...
        if( result.newBS ){
            ...
            // model variant = NotJust: can have illness from relapses even if none from the primary infection
            ...
            if( result.newPrimaryBS ){
                ...
                inf->setHadEvent( clinicalEvent );
                     ...
                     //double pFirstRelapseEvent = matImmClin * (pRelapseOneA * pRelapseOneB / (pRelapseOneB + (oldCumInf-1)));
                     ...
                     //double pSecondRelapseEvent = matImmClin * (pRelapseTwoA * pRelapseTwoB / (pRelapseTwoB + (oldCumInf-1)));
