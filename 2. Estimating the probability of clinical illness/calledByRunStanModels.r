...
# --- model B2 ----
model <- stan_model("modelA2_B2.stan")
fit <- sampling(
	model,
	list(
		Ncats=nrow(xtemp),
...
# save allchains 
if (paramSet==1) { write.table(out1, "output files/out1_B2_5p9param1_new.txt") }
if (paramSet==2) { write.table(out1, "output files/out1_B2_4p3param2_new.txt") }
...
# save allchains 
if (paramSet==1) { write.table(out2, "output files/out1_A2_5p9param1_new.txt") }
if (paramSet==2) { write.table(out2, "output files/out1_A2_4p3param2_new.txt") }
...
# --- model B1 -----
model <- stan_model("modelA1_B1.stan")
fit <- sampling(
	model,
	list(
		Ncats=nrow(xtemp),
...
# save results 
if (paramSet==1) { write.table(out3, "output files/out1_B1_5p9param1_new.txt") }
if (paramSet==2) { write.table(out3, "output files/out1_B1_4p3param2_new.txt") }
...
# save results
if (paramSet==1) { write.table(out4, "output files/out1_A1_5p9param1_new.txt") }
if (paramSet==2) { write.table(out4, "output files/out1_A1_4p3param2_new.txt") }
